...
# Load and scale proportionally
try:
    raw_skyline = load_image("skyline.png")
    sky_w, sky_h = raw_skyline.get_size()
    
    # We want the image to be exactly ROAD_FAR_Y (160px) high
    target_h = 160 
    scale_factor = target_h / sky_h
    
    # Calculate new width based on the scale factor to keep it looking correct
...
# --- NIGHT CITY PALETTE ---
# Dark Blues/Purples for the building body
BUILDING_DARK  = (10, 10, 25)
BUILDING_BASE  = (20, 20, 40)
BUILDING_SIDE  = (15, 15, 30)

# Window Colors
WIN_OFF        = (12, 12, 28)       # Almost black
WIN_WARM       = (255, 160, 50)     # Orange/Sodium lights
WIN_COOL       = (100, 200, 255)    # Cyan/Office lights
WIN_RED        = (255, 50, 50)      # Rare obstruction lights

# --- TERRAIN COLORS ---
GRASS_DARK  = (10, 10, 20)   # Deep night color
GRASS_LIGHT = (15, 15, 30)   # Slightly lighter band
SIDEWALK    = (40, 40, 50)   # Dark concrete
SIDEWALK_L  = (50, 50, 60)   # Lighter concrete
LANTERN_GLOW = (255, 200, 50) # Sodium street light color
TREE_COL    = (10, 40, 30)   # Dark teal for trees
...
def draw_background_and_terrain(surf, t_scroll):
    ...
    if IMG_SKYLINE:
        ...
        # Simple night sky fallback
        pygame.draw.rect(surf, (5, 5, 15), (0, 0, W, ROAD_FAR_Y))
    ...
    for i in range(bands):
        ...
        # --- A. DRAW GRASS/TERRAIN (The Dark Background) ---
        col_grass = GRASS_LIGHT if stripe == 0 else GRASS_DARK
        pygame.draw.rect(surf, col_grass, (0, int(y0), W, int(y1-y0)+1))

        # --- B. DRAW SIDEWALK (Strip next to road) ---
        # Sidewalk width shrinks as it goes into the distance
        sw_w0 = (right0 - left0) * 0.4  # Width factor
        ...
        poly_l = [
            (left0 - sw_w0, y0), (left0, y0),
            (left1, y1), (left1 - sw_w1, y1)
        ]
        # Right Sidewalk Polygon
        poly_r = [
            (right0, y0), (right0 + sw_w0, y0),
            (right1 + sw_w1, y1), (right1, y1)
        ]
...
def generate_building_surface(w, h, side):
    ...
    # 1. Main Body Background (Dark Blue gradient or solid)
    surf.fill(BUILDING_BASE)
    
    # 2. Draw a "Side" face to give 3D depth
    # If building is on Left (-1), shadow is on Left. If Right (1), shadow on Right.
    side_width = random.randint(5, 15)
    if side == -1:
        pygame.draw.rect(surf, BUILDING_SIDE, (0, 0, side_width, h))
        draw_area_x = side_width # Windows start after the side
    else:
        pygame.draw.rect(surf, BUILDING_SIDE, (w - side_width, 0, side_width, h))
        draw_area_x = 0 # Windows start at 0

    # 3. Window Generation
    # We restrict windows to the "Face" of the building
    face_w = w - side_width
    
    # Determine grid size (randomize for variety)
    ...
    for r in range(1, rows):
        # Specific logic for "lines" style (vertical stripes of light)
        ...
        for c in range(cols):
            ...
            # Decide if window is ON or OFF
            if style == "lines":
                lit = is_lit_col and random.random() < 0.9
            elif style == "office":
                lit = random.random() < 0.6 # Lots of lights
            else:
                lit = random.random() < 0.2 # Scattered
            
            if lit:
                # 80% Warm, 20% Cool, 1% Red
                rnd = random.random()
...
class Building:
    def __init__(self, side, z, layer=1):
        ...
        # Make back layer buildings slightly taller/wider on average
        ...
        # If Layer 2 (Background), darken the image to fake depth
        if self.layer == 2:
            darkener = pygame.Surface((base_w, base_h), pygame.SRCALPHA)
            darkener.fill((0, 0, 10, 100)) # Semi-transparent dark blue overlay
    ...
    def draw(self, surf):
        ...
        # --- OFFSETS BASED ON LAYER ---
        # Lanterns are at 10.
        # Layer 1 starts at 50 (leaving space for lanterns).
        # Layer 2 starts at 150 (way behind layer 1).
        if self.layer == 1:
            dist_from_road = 50 * scale
        else:
            dist_from_road = 180 * scale 

        if self.side == -1: 
            x = left_road - w - dist_from_road
        else: 
            x = right_road + dist_from_road
...
class SideObject:
    def __init__(self, side, z):
        ...
        self.z = z
        # We removed "kind" because now it's ONLY lanterns
    ...
    def draw(self, surf):
        ...
        # Position: CLOSE to the road (on the sidewalk)
        left_road, right_road = road_edges_at_y(y)
        offset_x = 10 * scale # Small offset so it sits on the curb
    ...
    while True:
        ...
        if started and alive and not paused:
            if counting_down:
                ...
                if building_spawn_progress > 0.10: # Slightly faster spawn rate
                    building_spawn_progress = 0
                    
                    # 1. Spawn Lanterns (Common, stick to road)
                    ...
                    # 2. Spawn Front Buildings (Layer 1)
                    ...
                    # 3. Spawn Back Buildings (Layer 2) - Denser background
