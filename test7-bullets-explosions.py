...
# Kerb colors (used in your draw_road)
KERB_RED = (200, 60, 60)
...
def y_from_z(z):
    # NOTE: z is clamped to 1.0, so objects stop moving down visually after 1.0.
    # We therefore despawn obstacles earlier (see main loop).
    z = clamp(z, 0.0, 1.0)
    t = ease_in(z)
...
def draw_road(surf, dash_offset=0.0):
    ...
    for i in range(num_segments):
        ...
        l_poly = [(l0 - curb_w0, y0), (l0, y0), (l1, y1), (l1 - curb_w1, y1)]
        pygame.draw.polygon(surf, col, l_poly)

        r_poly = [(r0, y0), (r0 + curb_w0, y0), (r1 + curb_w1, y1), (r1, y1)]
        pygame.draw.polygon(surf, col, r_poly)
...
class Obstacle:
    def __init__(self, lane, z, kind="car", sprite=None):
        ...
        # --- health for enemy cars ---
        if kind == "car":
            self.hp = random.randint(4, 5)
        else:
            self.hp = None
...
class Bullet:
    def __init__(self, lane, z):
        self.lane = lane
        self.z = z
        self.speed = 0.040  # bullet travels "forward" quickly
        self.radius = 4
    ...
    def draw(self, surf):
        x, y = self.get_pos()
        pygame.draw.circle(surf, (0, 255, 255), (x, y), self.radius)
...
class Explosion:
    def __init__(self, x, y):
        ...
        for _ in range(26):
            ...
            color = random.choice([(0, 255, 255), (255, 200, 50), (255, 80, 60), (255, 255, 255)])
            self.particles.append([x, y, vx, vy, radius, color])
    ...
    def update(self):
        ...
        self.particles = [p for p in self.particles if p[4] > 0.8]

    def draw(self, surf):
        # glow-y circles
        for x, y, vx, vy, r, col in self.particles:
            rr = int(r)
            if rr <= 0:
                continue
            s = pygame.Surface((rr * 4, rr * 4), pygame.SRCALPHA)
            pygame.draw.circle(s, (*col, 160), (rr * 2, rr * 2), rr * 2)
            pygame.draw.circle(s, (*col, 220), (rr * 2, rr * 2), rr)
            surf.blit(s, (int(x - rr * 2), int(y - rr * 2)))
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if alive and event.key == pygame.K_ESCAPE:
                    paused = not paused
                ...
                # Shoot (only when actually playing)
                if alive and (not paused) and event.key == pygame.K_SPACE:
                    if fire_cooldown == 0:
                        bullets.append(Bullet(player.lane, player.z - 0.08))
                        fire_cooldown = 10  # ~6 shots/sec
        ...
        if started and alive and not paused:
            ...
            if spawn_progress > spawn_threshold:
                ...
                for lane, kind in pattern:
                    sprite = None
                    if kind == "car":
                        sprite = IMG_ENEMIES[enemy_cycle_i] if IMG_ENEMIES else None
                        enemy_cycle_i = (enemy_cycle_i + 1) % max(1, len(IMG_ENEMIES))
            ...
            # obstacles update + despawn (remove shortly after passing player)
            p_rect = player.get_rect()
            for obs in obstacles[:]:
                obs.update(speed)

                # ✅ remove shortly after passing player to avoid "sticking" at bottom
                if obs.z > (player.z + 0.04):  # player.z is 0.92 => removes at 0.96
            ...
            for b in bullets[:]:
                ...
                for obs in obstacles[:]:
                    ...
                    if obs.get_rect().collidepoint(bx, by):
                        # consume bullet
                        bullets.remove(b)

                        # damage car
                        obs.hp -= 1
                        score += 40  # small hit score

                        # destroyed?
                        if obs.hp <= 0:
                            r = obs.get_rect()
                            explosions.append(Explosion(r.centerx, r.centery))
                            obstacles.remove(obs)
                            score += 300  # kill bonus
                        break
        ...
        if not started:
            ...
            bullets.sort(key=lambda b: b.z)

            # bullets first (usually farther), then obstacles
            for b in bullets:
                b.draw(frame)
            for obs in obstacles:
                obs.draw(frame)
            ...
            # show shooting hint
            draw_text_with_outline(frame, "SPACE: shoot", pygame.font.SysFont("Arial", 16), (255, 255, 255), (20, 88))
